...
#Copyright (c): 2016 Alex Buzunov. Free to use or distribute (star this repo).
...
----------------------------------------------------------------------
  ...
  postgresql_to_redshift_loader.exe [<pgres_query_file>] [<pgres_col_delim>] [<pgres_add_header>] 
	...
	--pgres_query_file -- SQL query to execure in source Oracle db.
...
assert PGPASSWORD, 'PGPASSWORD is not set.'
PGRES_CLIENT_HOME = os.getenv('PGRES_CLIENT_HOME') 
...
if __name__ == "__main__":		
	...
	start_time = time.time()
	assert opt.pgres_query_file, 'Input query file (-q,--pgres_query_file) is not set.'
	assert os.path.isfile(opt.pgres_query_file), 'Query file "%s"\ndoes not exists.' % opt.pgres_query_file
	q_file=os.path.splitext(os.path.basename(opt.pgres_query_file))
	
	assert opt.red_to_table, 'Target Redshift table '
	assert opt.s3_bucket_name, 'Target S3 bucket name (-b,--s3_bucket_name) is not set.'
	assert RepresentsInt(opt.pgres_lame_duck), '[-l] --pgres_lame_duck is not of type "integer".'
